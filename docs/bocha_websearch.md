# Bocha Web Search API Notes

This file is a provider API source note for `researcher/internal/provider/bocha`.

Implementation alignment:

- Default endpoint: `https://api.bocha.cn/v1/web-search`.
- Secret: `BOCHA_API_KEY`.
- Provider type: `direct_search`.
- Search results are leads, not final evidence.
- Prefer `datePublished`; normalize `dateLastCrawled` because the provider documents that it may represent UTC+8 time despite the `Z` suffix.
...
![](https://bocha-ai.feishu.cn/space/api/box/stream/download/asynccode/?code=Y2U1MDZjYzE2NGU4YWZlYzViY2E0MWRhYzUwYzg5ZmRfZ3FhdndKVlhFYVh3YXh5TXl5RjdFZXpRN3BEUk9mOEVfVG9rZW46Rjc4amIxN29pb1lFeE14dVZyN2N6bGk5bk9iXzE3NzkwMjg2NDg6MTc3OTAzMjI0OF9WNA)
...
| freshness | String  | 否      | 搜索指定时间范围内的网页。可填值：                                             |
| summary   | Boolean | 否      | 是否显示文本摘要。可填值：                                                 |
| include   | String  | 否      | 指定搜索的网站范围。多个域名使用`\|`或`,`分隔，最多不能超过100个可填值：例如：qq.com\|m.163.com |
| exclude   | String  | 否      | 排除搜索的网站范围。多个域名使用`\|`或`,`分隔，最多不能超过100个可填值：例如：qq.com\|m.163.com |
| count     | Int     | 否      | 返回结果的条数（实际返回结果数量可能会小于count指定的数量）。                             |
...
| ------- | --------------------------------------------------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------- |
| 403     | {  "code": "403",  "message": "You do not have enough money",  "log\_id": "c66aac17eab1bb7e"}       | 余额不足                           | 请前往 https://open.bochaai.com 进行充值                                                           |
| 400     | {  "code": "400",  "message": "Missing parameter query",  "log\_id": "c66aac17eab1bb7e"}            | 请求参数缺失                         |                                                                                             |
| 400     | {  "code": "400",  "message": "The API KEY is missing",  "log\_id": "c66aac17eab1bb7e"}             | 权限校验失败，Header 缺少 Authorization |                                                                                             |
| 401     | {  "code": "401",  "message": "Invalid API KEY",  "log\_id": "c66aac17eab1bb7e"}                    | API KEY无效                      | 权限校验失败                                                                                      |
| 429     | {  "code": "429",  "message": "You have reached the request limit",  "log\_id": "c66aac17eab1bb7e"} | 请求频率达到限制                       | 频率限制与总充值金额有关，具体详见：[ API 定价](https://aq6ky2b8nql.feishu.cn/wiki/JYSbwzdPIiFnz4kDYPXcHSDrnZb) |
| 500     | {  "code": "500",  "message": "xxxx",  "log\_id": "c66aac17eab1bb7e"}                               | 各种异常                           |                                                                                             |
